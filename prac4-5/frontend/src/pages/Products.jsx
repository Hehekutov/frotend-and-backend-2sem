...
export default function Products({ user }) {
  ...
  const handleAdd = async () => {
    if (!form.title || !form.category) return alert("Заполните все поля");
  ...
  const handleEdit = (product) => {
    ...
    setForm({ title: product.title, category: product.category, description: product.description, price: product.price, image: product.image || "" });
  };
    ...
    <div>
      <div className="form-container">
        ...
        <div className="form-buttons">
          ...
          {editId && <button onClick={()=>{setEditId(null); setForm({ title:"", category:"", description:"", price:"", image:"" })}}>Отмена</button>}
        </div>
      </div>
